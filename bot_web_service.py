...
# ---------------------------
# BINANCE CLIENT - INTEGRACIÓN COMPLETA CON API BINANCE FUTURES (AISLADO)
# ---------------------------
class BinanceClient:
    def __init__(self, api_key, api_secret):
        ...
        logger.info(f"Cliente Binance Futures inicializado con API Key: {api_key[:10]}...")
        self.isolated_mode = True  # Modo aislado por defecto
    ...
    def verificar_credenciales(self):
        ...
        try:
            logger.info("Verificando credenciales Binance Futures...")
            ...
            if account_info:
                ...
                logger.info(f"✓ Balance disponible: {available_balance:.2f} USDT")
                return True
            else:
    ...
    def get_account_info(self):
        """Obtener información de cuenta Binance Futures"""
        try:
            params = {
    ...
    def change_leverage(self, symbol, leverage):
        """Configurar apalancamiento (modo aislado por defecto)"""
        try:
            params = {
                'symbol': symbol,
                'leverage': leverage,
            ...
            if response.status_code == 200:
                ...
                if data.get('leverage') == leverage:
                    logger.info(f"✓ Apalancamiento {leverage}x configurado para {symbol} (aislado)")
                    return True
            logger.warning(f"Error configurando leverage: {response.text}")
            return False
        except Exception as e:
            logger.error(f"Error en change_leverage: {e}")
            return False

    def change_margin_type(self, symbol, margin_type='ISOLATED'):
        """Configurar tipo de margen (AISLADO por defecto)"""
        try:
            params = {
                'symbol': symbol,
            ...
            if response.status_code == 200:
                logger.info(f"✓ Tipo de margen {margin_type} configurado para {symbol}")
                return True
            else:
                # Si ya está configurado como aislado, Binance devuelve error pero es OK
                error_msg = response.text.lower()
                if 'no need to change margin type' in error_msg:
                    logger.info(f"✓ Tipo de margen ya está configurado como {margin_type} para {symbol}")
                    return True
            logger.warning(f"Error configurando margin type: {response.text}")
            return False
        except Exception as e:
            logger.error(f"Error en change_margin_type: {e}")
            return False

    def place_order(self, symbol, side, order_type, quantity, 
                    price=None, stop_price=None, reduce_only=False):
        """Colocar orden en Binance Futures (modo aislado)"""
        try:
            params = {
                'symbol': symbol,
                'side': side,
            ...
            if response.status_code == 200:
                data = response.json()
                logger.info(f"✓ Orden colocada: {data}")
                return data
            else:
                logger.error(f"Error en orden: {response.status_code} - {response.text}")
                return None
        except Exception as e:
...
# ---------------------------
# FUNCIONES DE OPERACIONES BINANCE
# ---------------------------
def ejecutar_operacion_binance(binance_client, simbolo, tipo_operacion, capital_usd=2, leverage=10):
    """
    Ejecutar una operación completa en Binance Futures (posición + TP/SL)
    
    Args:
        binance_client: Instancia de BinanceClient
        simbolo: Símbolo de trading (ej: 'BTCUSDT')
        tipo_operacion: 'LONG' o 'SHORT'
        capital_usd: Capital a usar en USD (default: 2)
        leverage: Apalancamiento (default: 10)
    """
    
    logger.info(f"🚀 EJECUTANDO OPERACIÓN REAL EN BINANCE FUTURES")
    ...
    try:
        # 1. Configurar tipo de margen como AISLADO
        margin_ok = binance_client.change_margin_type(simbolo, 'ISOLATED')
        if not margin_ok:
            logger.warning("No se pudo configurar margen aislado, continuando...")
        
        # 2. Configurar apalancamiento
        leverage_ok = binance_client.change_leverage(simbolo, leverage)
        if not leverage_ok:
        ...
        precio_actual = binance_client.get_price(simbolo)
        ...
        symbol_info = binance_client.get_symbol_info(simbolo)
        if not symbol_info:
            logger.error(f"No se pudo obtener info de {simbolo}")
            return None
        
        # 5. Calcular cantidad (2 USDT a 10x = 20 USDT de posición)
        valor_posicion = capital_usd * leverage  # 2 * 10 = 20 USDT
        cantidad = valor_posicion / precio_actual
        
        # Encontrar el step size para redondear
        ...
        min_qty = float(lot_size_filter.get('minQty', '0.001')) if lot_size_filter else 0.001
        if cantidad < min_qty:
            cantidad = min_qty
            logger.warning(f"Cantidad ajustada al mínimo: {cantidad}")
        
        logger.info(f"Cantidad: {cantidad}")
        logger.info(f"Valor nocional: ${cantidad * precio_actual:.2f}")
        
        # 6. Calcular TP y SL (2% SL, 4% TP - RR 2:1)
        ...
        # 7. Abrir posición (orden de mercado)
        orden_entrada = binance_client.place_order(
            symbol=simbolo,
            side=side,
            order_type='MARKET',
            quantity=round(cantidad, 8)
        )
        ...
        time.sleep(1)  # Esperar que se complete la orden
        
        # 8. Colocar Stop Loss (orden STOP_MARKET)
        sl_side = 'SELL' if tipo_operacion == 'LONG' else 'BUY'
        orden_sl = binance_client.place_order(
            symbol=simbolo,
            side=sl_side,
        ...
        # 9. Colocar Take Profit (orden TAKE_PROFIT_MARKET)
        orden_tp = binance_client.place_order(
            symbol=simbolo,
            side=sl_side,
            order_type='TAKE_PROFIT_MARKET',
            quantity=round(cantidad, 8),
            stop_price=round(take_profit, 2),
            reduce_only=True
        )
        ...
        logger.info(f"Valor posición: ${valor_posicion:.2f}")
...
# ---------------------------
# BOT PRINCIPAL - BREAKOUT + REENTRY CON INTEGRACIÓN BINANCE
# ---------------------------
class TradingBot:
    def __init__(self, config):
        ...
        # NUEVO: Inicializar cliente Binance si están las credenciales
        ...
        if config.get('binance_api_key') and config.get('binance_api_secret'):
            ...
            if self.binance_client.verificar_credenciales():
                logger.info("✅ Cliente Binance Futures inicializado y verificado")
            else:
                logger.warning("⚠️ No se pudieron verificar las credenciales de Binance")
        
        # NUEVO: Configuración de operaciones automáticas
        self.ejecutar_operaciones_automaticas = config.get('ejecutar_operaciones_automaticas', True)  # Por defecto SI
        self.capital_por_operacion = config.get('capital_por_operacion', 2)  # 2 USDT por defecto
        self.leverage_por_defecto = config.get('leverage_por_defecto', 10)  # 10x por defecto
    ...
    def obtener_datos_mercado_config(self, simbolo, timeframe, num_velas):
        """Obtiene datos con configuración específica usando API de Binance Futures"""
        if self.binance_client:
            try:
                klines = self.binance_client.get_klines(simbolo, timeframe, num_velas + 14)
                ...
                print(f"   ⚠️ Error obteniendo datos de Binance Futures para {simbolo}: {e}")
...
📍 {expectativa}
    ...
    def generar_grafico_breakout(self, simbolo, info_canal, datos_mercado, tipo_breakout, config_optima):
        ...
        try:
            ...
            # Usar API de Binance si está disponible
            if self.binance_client:
                klines = self.binance_client.get_klines(simbolo, config_optima['timeframe'], config_optima['num_velas'])
                if klines:
                    df_data = []
                    for kline in klines:
                        df_data.append({
...
💡 <b>Estrategia:</b> BREAKOUT + REENTRY con confirmación Stochastic
        ...
        if self.ejecutar_operaciones_automaticas and self.binance_client:
            print(f"     🤖 Ejecutando operación automática en Binance Futures...")
            try:
                operacion_binance = ejecutar_operacion_binance(
                ...
                if operacion_binance:
                    print(f"     ✅ Operación ejecutada en Binance Futures para {simbolo}")
                    # Enviar confirmación de ejecución
                    mensaje_confirmacion = f"""
🤖 <b>OPERACIÓN AUTOMÁTICA EJECUTADA - {simbolo}</b>
✅ <b>Status:</b> EJECUTADA EN BINANCE FUTURES (AISLADO)
📊 <b>Tipo:</b> {tipo_operacion}
💰 <b>Capital:</b> ${self.capital_por_operacion}
⚡ <b>Apalancamiento:</b> {self.leverage_por_defecto}x
🎯 <b>Entrada:</b> {operacion_binance['precio_entrada']:.8f}
🛑 <b>Stop Loss:</b> {operacion_binance['stop_loss']:.8f}
🎯 <b>Take Profit:</b> {operacion_binance['take_profit']:.8f}
💰 <b>Valor posición:</b> ${operacion_binance['valor_posicion']:.2f}
📋 <b>ID Orden:</b> {operacion_binance['orden_entrada'].get('orderId', 'N/A')}
⏰ <b>Timestamp:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
                    """
                    self._enviar_telegram_simple(mensaje_confirmacion, token, chat_ids)
                else:
                    print(f"     ❌ Error ejecutando operación en Binance para {simbolo}")
            except Exception as e:
        ...
        self.operaciones_activas[simbolo] = {
            ...
            'operacion_ejecutada': self.ejecutar_operaciones_automaticas and self.binance_client is not None
        }
...
💎 Integración: Binance Futures (AISLADO)
💰 Capital por operación: 2 USDT a 10x
...
🤖 Operación Binance: {operacion_ejecutada}
...
🕒 {datos_operacion['timestamp']}
    ...
    def generar_grafico_profesional(self, simbolo, info_canal, datos_mercado, precio_entrada, tp, sl, tipo_operacion):
        ...
        try:
            ...
            apds.append(mpf.make_addplot(oversold, color="#E9E4E4", linestyle='--', width=0.8, panel=1, alpha=0.5))
            fig, axes = mpf.plot(df, type='candle', style='nightclouds',
                               title=f'{simbolo} | {tipo_operacion} | {config_optima["timeframe"]} | Binance Futures + Breakout+Reentry',
                               ylabel='Precio',
                               addplot=apds,
                               volume=False,
                               returnfig=True,
    ...
    def mostrar_resumen_operaciones(self):
        ...
        if self.binance_client:
            print(f"   🤖 Binance Futures: ✅ Conectado (Modo AISLADO)")
        else:
            print(f"   🤖 Binance Futures: ❌ No configurado")
    ...
    def iniciar(self):
        ...
        print("🏦 INTEGRACIÓN: BINANCE FUTURES (MODO AISLADO)")
        print("=" * 70)
...
def crear_config_desde_entorno():
    ...
    return {
        ...
        # NUEVAS CONFIGURACIONES BINANCE
        'binance_api_key': os.environ.get('BINANCE_API_KEY'),
        'binance_api_secret': os.environ.get('BINANCE_SECRET_KEY'),
        'ejecutar_operaciones_automaticas': os.environ.get('EJECUTAR_OPERACIONES_AUTOMATICAS', 'true').lower() == 'true',
        'capital_por_operacion': float(os.environ.get('CAPITAL_POR_OPERACION', '2')),  # 2 USDT por defecto
        'leverage_por_defecto': int(os.environ.get('LEVERAGE_POR_DEFECTO', '10'))  # 10x por defecto
    }
...
@app.route('/')
def index():
    return "Bot Breakout + Reentry con integración Binance Futures (AISLADO) está en línea.", 200
