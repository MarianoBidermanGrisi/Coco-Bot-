# bot_web_service.py
# Adaptación para Render del bot Breakout + Reentry con Trading Real en Binance
import requests
import time
import json
import os
...
# --- FIN DE NUEVA LÓGICA DE TRADING ---
...
class TradingBot:
    def __init__(self, config):
        ...
        # --- INICIO: NUEVA LÓGICA DE TRADING ---
        # Cargar configuración de trading real desde variables de entorno
        ...
        if self.enable_real_trading:
            ...
            if not api_key or not secret_key:
                logger.error("❌ ENABLE_REAL_TRADING está en 'true', pero faltan BINANCE_API_KEY o BINANCE_SECRET_KEY.")
                self.enable_real_trading = False # Desactivar trading si faltan claves
            else:
                # Inicializar el trader. Comenzar en Testnet es lo más seguro.
                # Cambia 'testnet=True' a 'testnet=False' cuando quieras operar en real.
                self.binance_trader = BinanceTrader(api_key, secret_key, testnet=True)
                if not self.binance_trader.check_connection():
                    logger.error("❌ No se pudo conectar a la API de Binance. El trading real estará desactivado.")
                    self.binance_trader = None
                    self.enable_real_trading = False
        else:
            logger.info("ℹ️ Trading real está desactivado. El bot solo enviará señales a Telegram.")
        # --- FIN: NUEVA LÓGICA DE TRADING ---
...
📍 {expectativa}
    ...
    def generar_senal_operacion(self, simbolo, tipo_operacion, precio_entrada, tp, sl,
        ...
        """Genera y envía señal de operación con info de breakout y ejecuta trading real si está activado."""
...
💡 <b>Estrategia:</b> BREAKOUT + REENTRY con confirmación Stochastic
        ...
        # --- INICIO: NUEVA LÓGICA DE TRADING CON GESTIÓN DE RIESGO ---
        # Si el trading real está activado, colocar la orden en Binance
        if self.enable_real_trading and self.binance_trader:
            logger.info(f"🚀 Trading real ACTIVADO. Intentando abrir operación en Binance para {simbolo}.")
            
            try:
                # 1. Establecer el apalancamiento máximo de 5x
                leverage = 5
                self.binance_trader.set_leverage(simbolo, leverage)
                
                # 2. Obtener información de la cuenta para calcular el tamaño de la posición
                account_info = self.binance_trader.get_account_info()
                if not account_info:
                    logger.error(f"❌ No se pudo obtener información de la cuenta. Abortando operación en {simbolo}.")
                    return

                # 3. Calcular el tamaño de la posición basado en el 1% del margen disponible
                available_balance = float(account_info['availableBalance'])
                margin_to_use = available_balance * 0.01 # Usar el 1% del margen disponible
                
                # El valor nominal de la posición es el margen usado * el apalancamiento
                notional_value = margin_to_use * leverage
                
                # La cantidad a comprar/vender es el valor nominal / precio de entrada
                quantity_to_trade = notional_value / precio_entrada
                
                # Redondear la cantidad a la precisión del símbolo
                symbol_info = self.binance_trader.client.futures_exchange_info()['symbols']
                for s_info in symbol_info:
                    if s_info['symbol'] == simbolo:
                        precision = s_info['quantityPrecision']
                        quantity_to_trade = round(quantity_to_trade, precision)
                        break
                
                if quantity_to_trade <= 0:
                    logger.error(f"❌ La cantidad calculada para {simbolo} es 0 o negativa. Abortando.")
                    return

                logger.info(f"💰 Usando {margin_to_use:.2f} USDT de margen (1% de {available_balance:.2f} USDT).")
                logger.info(f"📈 Valor nominal de la posición: {notional_value:.2f} USDT con {leverage}x de apalancamiento.")
                logger.info(f"📊 Cantidad a operar: {quantity_to_trade} {simbolo.replace('USDT', '')}")

                # 4. Colocar la orden de mercado para abrir la posición
                side = 'BUY' if tipo_operacion == 'LONG' else 'SELL'
                order = self.binance_trader.place_market_order(simbolo, side, quantity_to_trade)

                if order:
                    # 5. Si la orden de mercado se ejecuta, colocar el Stop Loss
                    # El lado del Stop Loss es opuesto a la operación
                    stop_loss_side = 'SELL' if tipo_operacion == 'LONG' else 'BUY'
                    self.binance_trader.place_stop_loss_order(simbolo, stop_loss_side, quantity_to_trade, sl)

                    # 6. Guardar la información de la operación activa
                    self.operaciones_activas[simbolo]['order_id'] = order['orderId']
                    self.operaciones_activas[simbolo]['status'] = 'OPEN'
                    logger.info(f"✅ Operación en {simbolo} abierta y Stop Loss colocado.")
                else:
                    logger.error(f"❌ Falló la apertura de la operación en {simbolo}.")

            except Exception as e:
                logger.error(f"❌ Ocurrió un error inesperado durante el trading en {simbolo}: {e}")

        else:
            logger.info(f"ℹ️ Trading real desactivado. Solo se envió la señal a Telegram para {simbolo}.")
        # --- FIN: NUEVA LÓGICA DE TRADING CON GESTIÓN DE RIESGO ---
        ...
        self.operaciones_activas[simbolo] = {
            ...
            'order_id': self.operaciones_activas.get(simbolo, {}).get('order_id'),
            'status': self.operaciones_activas.get(simbolo, {}).get('status', 'SIGNAL_SENT')
        }
    ...
    def generar_reporte_semanal(self):
        ...
        reporte = f"""
📊 <b>REPORTE SEMANAL DEL BOT</b>
📅 Del {(datetime.now() - timedelta(days=7)).strftime('%Y-%m-%d')} al {datetime.now().strftime('%Y-%m-%d')}
🔢 Total de operaciones: {total_ops}
✅ Operaciones ganadoras (TP): {wins}
❌ Operaciones perdedoras (SL): {losses}
📈 Winrate: {winrate:.2f}%
💰 PnL Total: {pnl_total:.2f}%
📉 Promedio Ganadoras: {avg_ganancia:.2f}%
📉 Promedio Perdedoras: {avg_perdida:.2f}%
🏆 Mejor operación: {mejor_op['symbol']} ({mejor_op['tipo']}) con {mejor_op['pnl_percent']:.2f}%
💩 Peor operación: {peor_op['symbol']} ({peor_op['tipo']}) con {peor_op['pnl_percent']:.2f}%
        """
        return reporte

    # --- Métodos de ayuda para el análisis técnico ---
    ...
    def calcular_pearson_y_angulo(self, tiempos, precios):
        ...
        sum_t = sum(tiempos)
        sum_p = sum(precios)
        sum_tp = sum(tiempos[i] * precios[i] for i in range(n))
        sum_t2 = sum(t**2 for t in tiempos)
        sum_p2 = sum(p**2 for p in precios)
        denominator = math.sqrt((n * sum_t2 - sum_t**2) * (n * sum_p2 - sum_p**2))
        if denominator == 0:
            return 0, 0
        pearson = (n * sum_tp - sum_t * sum_p) / denominator
    ...
    def determinar_direccion_tendencia(self, angulo, min_strength=1):
        fuerza, nivel = self.clasificar_fuerza_tendencia(angulo)
        if nivel < min_strength:
    ...
    def calcular_stochastic(self, datos_mercado, k_period=14, d_period=3):
        ...
        if not stoch_k_values:
            return 50, 50
        k_smoothed = sum(stoch_k_values[-d_period:]) / d_period
        d_smoothed = sum(stoch_k_values[-(d_period * 2): -d_period]) / d_period if len(stoch_k_values) >= d_period * 2 else k_smoothed
    ...
    def generar_grafico_profesional(self, simbolo, info_canal, datos_mercado, precio_entrada, tp, sl, tipo_operacion):
        try:
            ...
            df['Resistencia'] = resistencia_values
            df['Soporte'] = soporte_values
            stoch_k, stoch_d = self.calcular_stochastic({'cierres': df['Close'].tolist(), 'maximos': df['High'].tolist(), 'minimos': df['Low'].tolist()})
            df['Stoch_K'] = [stoch_k] * len(df)
            df['Stoch_D'] = [stoch_d] * len(df)
            apds = [
                mpf.make_addplot(df['Resistencia'], color='red', linestyle='--', width=2, panel=0),
                mpf.make_addplot(df['Soporte'], color='green', linestyle='--', width=2, panel=0),
            ]
            if tipo_operacion == "LONG":
                tp_color = 'lime'
                sl_color = 'red'
            ...
            tp_line = [tp] * len(df)
            sl_line = [sl] * len(df)
            apds.append(mpf.make_addplot(tp_line, color=tp_color, linestyle=':', width=1.5, panel=0, secondary_y=False))
            apds.append(mpf.make_addplot(sl_line, color=sl_color, linestyle=':', width=1.5, panel=0, secondary_y=False))
            apds.append(mpf.make_addplot(df['Stoch_K'], color='blue', width=1.5, panel=1, ylabel='Stochastic'))
            apds.append(mpf.make_addplot(df['Stoch_D'], color='orange', width=1.5, panel=1))
            overbought = [80] * len(df)
            oversold = [20] * len(df)
            apds.append(mpf.make_addplot(overbought, color="gray", linestyle='--', width=0.8, panel=1, alpha=0.5))
            apds.append(mpf.make_addplot(oversold, color="gray", linestyle='--', width=0.8, panel=1, alpha=0.5))
            fig, axes = mpf.plot(df, type='candle', style='nightclouds',
                               title=f'{simbolo} | {tipo_operacion} | {info_canal["timeframe"]}',
                               ylabel='Precio',
                               addplot=apds,
                               volume=False,
                               returnfig=True,
    ...
    def enviar_grafico_telegram(self, buf, token, chat_ids):
        ...
        for chat_id in chat_ids:
            ...
            data = {'chat_id': chat_id, 'caption': "Análisis del mercado"}
            try:
...
# --- Inicialización y Ejecución del Servidor Flask ---
app = Flask(__name__)

# Cargar configuración desde variables de entorno para mayor seguridad
config = {
    ...
    'symbols': os.getenv('SYMBOLS', 'BTCUSDT,ETHUSDT,DOTUSDT,LINKUSDT,BNBUSDT,XRPUSDT,SOLUSDT,AVAXUSDT,DOGEUSDT,LTCUSDT,ATOMUSDT,XLMUSDT,ALGOUSDT,VETUSDT,ICPUSDT,FILUSDT,TRXUSDT,XTZUSDT,SUSHIUSDT,ZILUSDT,ENJUSDT,ZECUSDT').split(','),
...
# Instanciar el bot globalmente
bot = TradingBot(config)

@app.route('/', methods=['GET', 'POST'])
def run_bot():
    try:
        bot.escanear_mercado()
        bot.guardar_estado()
        return jsonify({"status": "success", "message": "Escaneo completado."})
    except Exception as e:
