...
# ✅ CORREGIDO: get_price_precision ya no devuelve valores negativos (evita "Format specifier missing precision")
import requests
import time
import json
import os
...
import threading
import logging
# --- MÓDULO BINANCE TRADER (EMBEDDED) ---
...
📍 {expectativa}
    ...
    def calcular_tamaño_posicion(self, symbol, precio_entrada):
        ...
        cantidad_ajustada = round(cantidad_base, precision)
        if cantidad_ajustada < step_size:
        ...
        for simbolo in self.config.get('symbols', []):
            try:
                if simbolo in self.operaciones_activas:
                    print(f"   ⚠️ {simbolo} - Operación ya activa. Saltando análisis.")
                    continue
...
💰 Entrada: {datos_operacion['precio_entrada']:.{precision}f}
🎯 Salida: {datos_operacion['precio_salida']:.{precision}f}
💵 PnL Absoluto: {pnl_absoluto:.{precision}f}
